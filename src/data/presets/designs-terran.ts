...
export const terranDesigns: ShipDesignInput[] = [
  ...
  },
  {
    ...
    // A perimeter armour belt (dorsal/ventral caps + closed prow face around the
    // centre laser) replaces what was a thin four-cell cap; the missile and
    // sensor arcs fire through the belt (own hull never occludes equipment).
    grid: subdivideGrid(withEdges(gridFromMap([
      "..#####",
      "..J~Mv#",
      "E=CFMM#",
  ...
  },
  {
    ...
    // quarters, and an engine bank. The Mk I shields protect the flanks; a
    // dorsal/ventral armour belt wraps the hull and a light armour shoulder (#)
    // caps each prow corner ahead of the railguns.
    grid: mountMultiCell(
      subdivideGrid(withEdges(gridFromMap([
        ...
        { col: 4, row: 4, dir: "e", kind: "door" },
        { col: 4, row: 5, dir: "e", kind: "wall" },
      ]), F_GUNSHIP),
      F_GUNSHIP,
      [
        // Twin Pulse Array on the central deck corridor (wingtip battery
        // upgrade), Twin Rail Turret on the lower deck corridor.
        [4, 2, "ter-twin-pulse-array", TERRAN_FOOTPRINTS.twinPulseArray],
        [4, 4, "ter-twin-rail-turret", TERRAN_FOOTPRINTS.twinRailTurret],
      ],
    ),
    createdAt: PRESET_TIME,
    updatedAt: PRESET_TIME,
  ...
  },
  {
    ...
    // armour anchor) and lets it screen a Leviathan against railgun spam. A
    // full perimeter armour belt backs the shields so a downed shield face
    // still leaves a plate under it. No finite-ammo weapons means no magazine
    // required; fusion reactors and a deep crew and engine block run it.
    grid: mountMultiCell(
      subdivideGrid(withEdges(gridFromMap([
        "########",
        "#>JWS~L#",
      ...
      [
        ...
        [6, 2, "ter-damage-control-bay", TERRAN_FOOTPRINTS.damageControlBay],
      ],
    ),
    createdAt: PRESET_TIME,
    updatedAt: PRESET_TIME,
  ...
  },
  {
    ...
    // prow of ablative plating, thickened into a solid cap, now wrapped by a
    // full perimeter armour belt. Deflector Mk I screens stop the kinetics that
    // would otherwise strip the plate, while railgun turrets answer over the
    // top. The magazine (G) in the central corridor feeds all railguns; slow,
    // unyielding, the thing you park in front of a Leviathan. This is the clean
    // split with the Bulwark: Bulwark = shields + mobility, Aegis = plate +
    // momentum screen.
    grid: mountMultiCell(
      subdivideGrid(withEdges(gridFromMap([
        "#########",
      ...
      [
        ...
        [5, 5, "ter-mine-layer-bank", TERRAN_FOOTPRINTS.mineLayerBank],
      ],
    ),
    createdAt: PRESET_TIME,
    updatedAt: PRESET_TIME,
  ...
  },
  {
    ...
    // needed to man torpedoes and missiles; a perimeter armour belt and titanium
    // plate (#) on the prow shoulders and core, plus shields, protect the
    // fragile innards.
    grid: mountMultiCell(
      subdivideGrid(withEdges(gridFromMap([
        "#########",
      ...
      [
        ...
        [4, 1, "ter-broadside-missile-bank", TERRAN_FOOTPRINTS.broadsideMissileBank],
      ],
    ),
    createdAt: PRESET_TIME,
    updatedAt: PRESET_TIME,
  ...
  },
  {
    ...
    //
    // Layout (13 cols × 7 rows), subdivided ×7 → 91 m cruiser:
    // stern (left) → crew/reactor spine → corridors → magazine → weapons → prow.
    // A dorsal/ventral armour belt hugs the hull and prow cap (cols 0-7, 10);
    // the stepped arrowhead prow (cols 10-12) is left open at its base so the
    // belt follows the existing silhouette rather than filling the weapon face.
    grid: mountMultiCell(
      subdivideGrid(withEdges(gridFromMap([
        "########..#..",
        "#>JWUTRL..#..",
      ...
      F_LEVIATHAN,
      [
        [7, 3, "ter-spinal-lance", TERRAN_FOOTPRINTS.spinalLance],
        // Light Spear Lance on the freed upper railgun cell — a fixed spinal
        // secondary beam for the cruiser.
        [6, 2, "ter-light-spear-lance", TERRAN_FOOTPRINTS.lightSpearLance],
        // Flak Bastion on the freed prow railgun cell — capital PD coverage.
        [8, 3, "ter-flak-bastion", TERRAN_FOOTPRINTS.flakBastion],
  ...
  },
  {
    ...
    //
    // Layout (13 cols × 7 rows), subdivided ×12 → 156 m dreadnought:
    ...
    // A dorsal/ventral armour belt wraps the hull; the upper prow-cap shoulder
    // (col 11) is closed to mirror the lower, so the stepped arrowhead prow
    // (cols 11-12) reads symmetrically. Belt cells hug the existing silhouette.
    grid: mountMultiCell(
      subdivideGrid(withEdges(gridFromMap([
        ".###########.",
        ".#>JWkvRML##.",
      ...
      [
        ...
        [7, 3, "ter-spinal-driver", TERRAN_FOOTPRINTS.spinalDriver],
        [8, 3, "ter-heavy-railgun-turret", TERRAN_FOOTPRINTS.heavyRailTurret],
        // Port stern: a three-cell Capital Plasma Drive (B) replaces the ion
        // drive as the main propulsion.
        [1, 2, "ter-capital-drive", TERRAN_FOOTPRINTS.capitalDrive],
        ...
        [1, 3, "ter-cross-reactor", TERRAN_FOOTPRINTS.crossReactor],
        // Stern shield: the 2×2 Bastion Shield Array (N) replaces the Mk II.
        [6, 5, "ter-bastion-shield", TERRAN_FOOTPRINTS.bastionShield],
        // Prow momentum screen: the two-cell Bulwark Deflector (k).
        [5, 1, "ter-bulwark-deflector", TERRAN_FOOTPRINTS.bulwarkDeflector],
        ...
        [6, 3, "ter-plus-reactor", TERRAN_FOOTPRINTS.plusReactor],
        // Compass Drone Hangar on the lower-deck corridor — drone capability,
        // a new doctrine angle for the Titan.
        [6, 4, "ter-drone-hangar", TERRAN_FOOTPRINTS.droneHangar],
      ],
    ),
    createdAt: PRESET_TIME,
    updatedAt: PRESET_TIME,
