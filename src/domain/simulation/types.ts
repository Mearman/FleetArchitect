...
export interface BattleInputs {
   ...
   * with no fixed tick limit until a side is eliminated (a modular ship that
   * loses every reactor is destroyed, so a mutual brownout resolves rather than
   * stalling). Focused tests may pass a small cap to stop a battle early without
   * running it out.
   */
  maxTicks?: number;
  /**
